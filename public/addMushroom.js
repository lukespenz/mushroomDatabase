...
const addForm = document.getElementById('add-mushroom-form') 
...
// const addMushroom = (event) => {
//     event.preventDefault()

//     console.log('addMushroom invoked', event) //?? not being invoked ??
//     axios.post(`${addPage}`, event).then().catch(errCallback)
// }
const deleteMushroom = (id) => axios.delete(`${viewPage}/${id}`).then().catch(errCallback)
...
const submitHandler = (e) => {
    ...
    let sporePrint = document.querySelector('#spore_print')
    ...
    axios.post(addPage, mushroomInfo).then(() => {
...
const createMushroomCard = (mushroom) => {
        ...
        addForm.addEventListener('submit', submitHandler) //when add mushroom button pressed it invokes addMushroom function
